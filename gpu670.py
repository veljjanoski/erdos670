...
void anneal(float* Xall, int n, int d, int iters, float T0, float T1, unsigned long long seed, float* bestF, float* bestX) {
    ...
    float gmin = 1e30f; for (int a = 0; a + 1 < N; a++) { float g = S[a+1] - S[a]; if (g < gmin) gmin = g; }
    float cur = (gmin > 1e-9f) ? S[N-1] / gmin : 1e30f;
    ...
    for (int it = 0; it < iters; it++) {
        ...
        gmin = 1e30f; for (int a = 0; a + 1 < N; a++) { float g = S[a+1] - S[a]; if (g < gmin) gmin = g; }
        float nw = (gmin > 1e-9f) ? S[N-1] / gmin : 1e30f;
...
def search(n, d, threads, iters, rounds, rng, verbose=True, seed_golomb=True):
    ...
    for r in range(rounds):
        ...
        for e in range(8):
            try:
                x, D = polish(elite[e].ravel(), n, d, iu, rounds=40); v = feasible_value(x, n, d, iu)
